// We add promote and demote menuitems for the toc in a new menu.
// The tricky part is to enable them and disable them at the
// correct times.  The method enableDisableButtons, slightly
// misnamed, helps in this regard.
...
import com.esri.mo2.map.draw.*;// BaseSimpleRenderer,SimplePolygonSymbol
...
public class QuickStart9d extends JFrame {
  ...
  public QuickStart9e() {
    ...
    layerlis = new ActionListener() {public void actionPerformed(ActionEvent ae){
	  ...
	  if (source instanceof JMenuItem) {
		...
		if(arg == "add layer") {
		  ...
		  SimplePolygonSymbol simplepolysymbol = new
		    SimplePolygonSymbol();// for polygons
    ...
    mytocadapter = new TocAdapter() {
	  public void click(TocEvent e) {
	    ...
	    legend = e.getLegend();
	    // sometimes clicking on a legend deselects it, so then
	    // no layer is selected, or more accurately, no
	    // layer should be considered 'active'
	    // it is possible but unlikely to have more than one
	    //   layer be regarded as selected at a given time;since
	    //   at most one layer can be active, we pay more
	    //   attention to that
	    System.out.println("is selected is "+legend.isSelected());
  ...
  public static void main(String[] args) {
    QuickStart9e qstart = new QuickStart9e();
	  ...
	  public void actionPerformed(ActionEvent ae) {
	    ...
	    if (source == cancel)
	      ...
	      try {
			...
			map.redraw();
			if (QuickStart9e.stb.getSelectedLayers() != null){
			  QuickStart9e.promoteitem.setEnabled(true);}
		  } catch(IOException e){}
	    }
	  }
    };
...
class AttrTab extends JDialog {
  JPanel panel1 = new JPanel();
  com.esri.mo2.map.dpy.Layer layer = QuickStart9e.layer4;
...
class CreateShapeDialog extends JDialog {
  ...
  ActionListener lis = new ActionListener() {public void actionPerformed(ActionEvent ae) {
	...
	if (o == nameField) {
	  ...
	  path = ((ShapefileFolder)(QuickStart9e.layer4.getLayerSource())).getPath();
